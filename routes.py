...
async def extract_endpoint(payload: ExtractRequest):
    try:
        ...
        return {"status": "success", "length": len(raw_text), "text": raw_text}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def query_endpoint(payload: QueryRequest):
    try:
        ...
        return {
            "status": "success",
            "matches": [
                {"score": round(score, 4), "text": chunk} for chunk, score in results
            ],
        }
...
async def answer_endpoint(payload: AnswerRequest):
    try:
        ...
        return {"status": "success", "answer": answer, "context_used": context}
