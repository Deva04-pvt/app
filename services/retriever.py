...
def search_faiss_index(index: faiss.IndexFlatL2, query: str, chunks: List[str], top_k: int = 5) -> List[Tuple[str, float]]:
    # Embed the query (Gemini returns batch even for single input)
    query_embedding = get_embeddings_gemini([query])[0]
